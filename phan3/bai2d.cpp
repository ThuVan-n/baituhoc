...
// Ham nhap thong tin cua sinh vien
...
// Ham tim sinh vien co ho la "Nguyen"
void timSinhVienNguyen(char fullName[][MAX_LEN], int soSinhVien) {
    cout << "Cac sinh vien co ho la 'Nguyen':\n"; // Sinh vien co ho la "Nguyen":
    for (int i = 0; i < soSinhVien; ++i) {
        if (strstr(fullName[i], "nguyen") != NULL) { // Neu ho la "Nguyen"
            cout << fullName[i] << endl; // Xuat ra man hinh ho ten
        }
    }
}
...
int main()
{
    ...
    nhapSinhVien(fullName, soSinhVien); // Nhap thong tin cua sinh vien

    timSinhVienNguyen(fullName, soSinhVien); // Tim sinh vien co ho la "Nguyen"
